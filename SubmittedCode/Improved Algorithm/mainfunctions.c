			...
			if(Lset[Lpoint] >= Rset[Rpoint]){	//If L > R, we need to increase R so that R > L.
				...
				for (int i = startpoint; i < intervalsize; i++){	//Going through each letter in the interval checking for an existing expanding letter.
					for(int j = 0; j < Esize; j++){ 				
						if(word[i] == Eset[j].letter){				//Check if the letter exist in the expanding set
							Efind = TRUE;
							break;
						}
					}
					if(Efind == TRUE){			//Found expanding letter
						Letterfind = FALSE;		//The interval doesn't contain a new expanding letter
						minocc = lenw+1;		//Reset the minimum occurence data to pervent any error carry toward
						break;
					}
					else{
						...
						if( minocc > tempocc){							//Compare it to the previous minimum occurrence
							minocc = tempocc;							//Update the new minimum occuring letter in this interval
							minchar = tempchar;
							loc = temploc;
						}
						Letterfind = TRUE;	//This interval potentially doesn't contain any expanding letter
					}
				}
				if(Letterfind == TRUE){	//If this is true, this means these exist an interval that doesn't contain any expanding letter
					intervalfind = intervalfind + 1;
					break;	//Need to test without this.
				}
				Lpoint = Lpoint + 1;	//Check the next interval
